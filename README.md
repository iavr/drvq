...
Because of the hierarchical structure and the custom binary file format, codebooks produced by `train` can only be read by the `drvq` library and tools. But codebooks are essentially sets of centroids (vectors), so tool `flat`, specified by [flat.cpp](/src/flat.cpp), "flattens" and exports centroids in a format that can be read e.g. by [load_double_array.m](/matlab/load_double_array.m) as a two-dimensional matrix in Matlab. These vectors can then be used in any application but without the fast encoding capabilities of `drvq`.
